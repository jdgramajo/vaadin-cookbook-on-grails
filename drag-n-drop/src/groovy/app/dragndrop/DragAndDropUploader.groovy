...
public class DragAndDropUploader extends VerticalLayout {
	private static final String REPOSITORY = "/Users/jgramajo/Desktop/"
	...
	public DragAndDropUploader() {
		...
		dropTable.setDropHandler(new DropHandler() {
			...
			public void drop(DragAndDropEvent event) {
				...
				def files = transferred.getFiles()
				if (files) {
					files.each { file ->
						ProgressIndicator indicator = new ProgressIndicator()
						indicator.setPollingInterval(100)
						indicator.setSizeFull()
						table.addItem([file.getFileName(), getSizeAsString(file.getFileSize()), 
							indicator] as Object[], null)
						StreamVariable streamVariable = createStreamVariable(file, indicator)
	...
	private StreamVariable createStreamVariable(final Html5File file,
		...
		return new StreamVariable() {
			...
			public void onProgress(StreamingProgressEvent event) {
				indicator.setValue((float) event.getBytesReceived() / file.getFileSize())
			}
